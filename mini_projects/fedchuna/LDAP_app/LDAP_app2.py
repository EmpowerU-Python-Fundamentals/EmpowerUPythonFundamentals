...
class LD(tk.Toplevel):
    ...
    def __init__(self, master=None):
        ...
        # print(f"Bind Password: {self.bind_password}")
        # print(f"Base DN: {self.base_dn}")
        #TODO: write module to pars, OATH data from conf.file
        if self.l is None:
            btn1 = tk.Button(self, text="Connect LDAP", command=lambda: m_l.bind(self.ldap_server, self.l , self.bind_dn, self.bind_password))
            btn1.pack(pady=5)
            self.l = m_l.bind(self.ldap_server, self.l , self.bind_dn, self.bind_password)
        else:
            btn3 = tk.Button(self, text="Disconnect Ldap", command=lambda: m_l.close_bind(self.l))
            btn3.pack(pady=5)
            self.l = m_l.close_bind(self.l)
    ...
    def get_oath_data(self):
        try:
            with open(self.ldap_conf_pass, 'r', encoding='utf-8') as conf_f:
                self.ldap_server = conf_f.readline().strip()
                self.bind_dn = conf_f.readline().strip()
                self.bind_password = conf_f.readline().strip()
                self.base_dn = conf_f.readline().strip()
                self.ldap_server = self.ldap_server.replace('LDAP server: ', '').strip()
                self.bind_dn = self.bind_dn.replace('Bind DN: ', '').strip()
                self.bind_password = self.bind_password.replace('Bind Password: ', '').strip()
                self.base_dn = self.base_dn.replace('Base DN: ', '').strip()
            ...
            print(f"Configuration file {self.ldap_conf_pass} not found.")
